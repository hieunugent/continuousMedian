...
class Heap:
	...
	def siftDown(self, currentIdx, endIdx, heap):
		...
		while childOneIdx <= endIdx:
			...
			if childTwoIdx != -1:
				if self.comparisionFunc(heap[childTwoIdx], heap[childOneIdx]):
					idxToSwap = childTwoIdx
				else:
					idxToSwap = childOneIdx
			else:
				idxToSwap = childOneIdx
			if self.comparisionFunc(heap[idxToSwap], heap[currentIdx]):
	...
	def siftUp(self, currentIdx, heap):
		...
		while currentIdx > 0:
			if self.comparisionFunc(heap[currentIdx], heap[parentIdx]):
